...
A robot \robot is usually composed of a set of rigid bodies \body{i},
$i \in 0..N_B$, and a set of joints \joint{i}, $i \in 1..N_J$, which
...
than one configuration of \robot\thinspace that allows \body{e} to
reach the target. The robot \robot\thinspace is therefore
...
We assume here that the robot \robot\thinspace is walking on a flat
...
contacts are non-sliding. \robot\thinspace is then subject to its
...
\robot\thinspace are zero. It is also established that
\robot\thinspace will not tilt around its foot edges as long as the
...
the robot \robot\thinspace is then said to be dynamically balanced. We
...
ends starting from $\mathbf{q}_s$, and is denoted
$\mathbf{q}_n^{init}$.
...
this speed constraint along the edge.
...
$\hat{h}(\mathbf{q}_n^{state})$ must verify
$\hat{h}(\mathbf{q}_n^{state}) \leq h(\mathbf{q}_n^{state})$ to ensure
that the algorithm is admissible, i.e. the path from $\mathbf{q}_s$ to
$\mathbf{q}_g$ is optimal. Since the robot is fastest while walking
straight forward in the absence of obstacles,
$\hat{h}(\mathbf{q}_n^{state})$ is defined as:
\begin{equation}
  \begin{split}
...
the HRP-2 had lifted its arms over the two chairs. Besides leading to
