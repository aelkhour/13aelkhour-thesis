...
tasks for redundant systems, the set of solution configurations has a
specific topological structure and forms a differentiable submanifold
$\manifold$ of {\cspace}, i.e.\ a set that locally ``looks like'' the
euclidean space \tspace{m}, and such that the tangent vector space
...
implementation, joint limit position bounds are not enforced. Clamping
the update at each iteration allows making sure they are not violated.
...
planning techniques based on higher-\linebreak dimensional continuation.
...
\begin{enumerate}
\item Sample a random configuration \config{rand} in {\cspace} with
...
\caption[Random goal configurations solving a reaching task.]{Random
  goal configurations solving a reaching task. All the configurations
  are balanced and collision-free, and the right hand of the robot
...
\begin{figure}
  ...
  \caption[One step of extension of the RRT algorithm.]{One step of
    extension of the RRT algorithm. The algorithm tries to add the
    longest possible edge from \config{near} towards \config{rand},
...
probabilistically complete. This proof equally applies to this
algorithm.
...
(434.3656,81.6056) node [above = 0.1cm, color=dg, text width=2.cm] {\small{Constrained manifold $\manifold$}};
...
\caption[One step of constrained extension.]{One step of constrained
  extension illustrating Algorithm \ref{alg:constrained}:
  \config{rand} is first projected on $T_{\mathbf{q}}\manifold$ the
  tangent space of $\manifold$. \config{rand}$'$ is then projected
  onto $\manifold$ at \config{rand}$''$. A classic RRT extension tries
  to go as far as possible from \config{near} towards
  \config{rand}$''$ while remaining on $\manifold$. \config{new} is
  then returned.}
\label{fig:gikrrt}
\end{figure}
...
\caption[HRP-2 displaces a ball in a shelf.]{HRP-2 grabs a ball on a
  shelf, puts it on another shelf, and comes back to a rest
  position. Static balance constraints are enforced along the path,
  and the intermediary goals consisting in grasping and displacing the
  ball are defined implicitly as inverse kinematics constraints.}
...
\hline
\end{tabular}
\caption[Experimental results on 20 motion planning
  runs.]{Experimental results on 20 runs: Each run consists of 3
...
\config{1} and \config{2}, there exists a trajectory going from
\config{1} to \config{2} in a finite time interval. It is
\textit{small-space controllable} if for all configurations \config{},
for all $\epsilon >0$, there exists $\eta >0$ such that all the
configurations contained in the ball of center \config{} and radius
$\eta$ are reachable, in a finite time interval, by trajectories
included in the ball of center \config{} and radius
$\epsilon$. Figure~\ref{fig:ssc1} shows an illustration of this
property.
...
\begin{figure}
  ...
  \caption[The small-space controllability property.]{The
    small-space controllability local property: any configuration $q'$
    at a distance less than $\eta$ is reachable from \config{} by an
    admissible trajectory included in a ball of size $\epsilon$.}
  \label{fig:ssc1}
\end{figure}
...
\begin{figure}
  ...
  \caption[Small-space controllability in motion
    planning.]{Small-space controllability in motion planning.  A
    collision-free path from \config{1} to \config{2} is approximated
    by collision-free and admissible trajectories by using the local
    property.  }
  \label{fig:ssc2}
\end{figure}
...
small-time controllability property. However, the converse is not
...
robot which can be fixed or sliding on the ground.
...
\begin{figure}
  ...
  \caption[Collision-free path for a sliding humanoid
    robot.]{Collision-free statically balanced path for a humanoid
    robot sliding on the ground.}
  \label{fig:sliding}
\end{figure}
...
\subsection{Small-Space Controllability of Dynamically Walking Robots}
...
\config{} and radius $\epsilon$ $B($\config{}$,\epsilon)$, is such
that the CoM is never over the left foot.  Given the quasi-static
walking conditions, the right foot of the robot is fixed along the
trajectory. Thus, the set of accessible configurations from \config{}
by staying inside $B($\config{}$,\epsilon)$ does not form a
neighborhood of \config{}, since it does not include any configuration
where the right foot has moved. This shows that the robot is not
small-space controllable.
...
\begin{figure}
  ...
  \caption[Simplified model of a legged robot.]{Simplified model of a
    legged robot. The CoM is at $(0,0,z_c)$, the two feet are flat on
    the ground, aligned with the $y$-axis, at a horizontal distance
    $L$ from the CoM.}
...
generates a discontinuity in the velocity of the CoM at time $(t=0)$. To
...
$\dot{f}(0) = 0$. When $t\geq T$, the robot is in the steady state
...
\caption[CoM motion (solid line) along $y$ axis.]{CoM motion (solid
  line) along $y$ axis. The CoM stays in the interval
  $[-\epsilon,\epsilon]$ while during steady state ($t \geq T$), the
  ZMP (dashed line) oscillates between the centers of the feet, which
  allows in-place walk.}
...
axis and the corresponding ZMP motion. Once in steady in-place
...
We have not extended the above proof to any legged robot model
since empirically, the table cart model describes a large part of the
...
The control strategy presented in the above proof may generate very
...
called Generalized inverse kinematics (Gik) developed in
\cite{yoshida2006tds}.
...
\begin{figure}
  \centering
      ...
      \caption[The first steps of the walk planning algorithm.]{The
        first three steps of algorithm \ref{alg:walk} are shown (top
        left and right, bottom left). Starting from a sliding
        collision-free path (dashed), a walking trajectory is computed
        using nominal walk parameters. If collisions are detected, the
        path is recursively cut and animated until a collision-free
        dynamic walking trajectory is found (bottom right). Notice
        that shorter and faster steps in the middle of the trajectory
        lead to smaller deviations of the CoM trajectory (in red) from
        the original path.}
...
\caption[The robot HRP-2 passing between two chairs.]{The robot HRP-2
  passing between two chairs. In this kind of environment whole-body
  collision avoidance is needed during locomotion.}
\label{fig:couv}
\end{figure}
...
\caption[Horizontal trajectory of the robot CoM during
  locomotion.]{Horizontal trajectory of the robot CoM during
...
\caption[Solution path for a cluttered environment.]{Solution path for
  a cluttered environment, the robot walks among floating obstacles.}
\label{fig:cluttered}
\end{figure}
...
\caption[Solution path for a hand reaching problem in an
  apartment.]{Solution path for a hand reaching problem in an
...
\caption[Execution of the walking trajectory by HRP-2 on
  stage.]{Execution of the walking trajectory by HRP-2 on stage. The
