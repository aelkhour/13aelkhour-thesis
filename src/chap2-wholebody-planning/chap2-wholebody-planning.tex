...
algorithm, from a configuration already in the tree \config{rand} towards
...
\begin{figure}
  ...
  \begin{tikzpicture}[x=0.45cm,y=0.45cm]
    ...
    \node [above left] at (qn) {\config{rand}};
  ...
  \caption{One step of extension of the RRT algorithm. The algorithm
    tries to add the longest possible edge from \config{rand} towards
...
this extension. The idea is to first project \config{rand} on the
tangent space to $\manifold$ at \config{rand}. Let us call the
projected configuration \config{rand}$'$. Let \config{rand}$''$ be the
result of a call to $\texttt{SolveConstraints}($\config{rand}$',f,\epsilon)$.
It is the projection of \config{rand}$'$ on $\manifold$. Instead of
extending the tree from \config{rand} towards \config{rand}, the
algorithm tries to extend from \config{rand} towards \config{rand}$''$
...
\node [above = 0.15cm] at (qn) {\config{rand}};
...
\caption{One step of constrained extension illustrating Algorithm
  ...
  $T_{\mathbf{q}}\manifold$ the tangent space of $\manifold$. \config{rand}$'$ is then
  projected onto $\manifold$. A classic RRT extension tries to go as
  far as possible from \config{rand} towards \config{rand}$''$ while remaining
  on $\manifold$. \config{new} is then returned.}
\label{fig:gikrrt}
\end{figure}
