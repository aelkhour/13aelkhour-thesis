...
\begin{figure}
  \centering
      ...
      \caption{FIXME:Optimal control problem}
...
\ref{eq:chap3-hjb} is that PDE that gives the necessary and sufficient
...
hybrid: Controls and node start values in NLP

Discretize control piecewise on a coarse grid
$\mathbf{u}(t)=\mathbf{q}_i$ for $t\in\left[t_i,t_{i+1}\right]$

Solve ODE on each interval $\left[t_i,t_{i+1}\right]$ numerically,
starting with an artificial initial state value $\mathbf{s}_i$ such that:
...
Obtain trajectory pieces $\mathbf{x}_i(t,\mathbf{s}_i,\mathbf{q}_i)$,
as shown in Figure
\ref{fig:chap3-optimal-control-problem-multiple-shoot}.
...
Introduce continuity conditions to ensure state continuity over the
whole duration.
...
and $\mathcal{L}$ is the Lagrangian of $\mathbf{F}$.

If we have a closer look at the first equality constraint, we notice
...
\mathbf{X}_i^\mathbf{q}\Delta\mathbf{q}_i - \Delta\mathbf{s}_{i+1}, \quad i=0,\ldots,N-1\\
\Updownarrow\\
...
To conclude, multiple-shooting direct methods offer several
advantages: they rely on a coarse discretization of both control and
state vectors, using adaptive-step ODE solvers to integrate on the
sub-intervals. They can thus use knowledge of the state in
initialization, which makes very suitable for applications where a
good initial guess of the state can be given. Furthermore, the
...
takes advantage of the QP subproblem sparsity and solve them
efficiently.
...
\textsc{IPOPT}
\textsc{CHOMP} (different than other?)
