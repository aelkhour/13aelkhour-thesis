...
* Time-Optimal motions motions: time parametrization to minimize
objective function \cite{verscheure2009time,suleiman2010time}
minimum-time: time parametrization on existing path, no geometric
reshaping \cite{suleiman2008human}

* Motion imitation

* Obtain movements that would be very hard or impossible to generate
otherwise \cite{miossec2006development} and \cite{arisumi2008dynamic}:
go out of static feasibility domain to achieve goal.

* \emph{Model Predictive Control (MPC)}, the current control depends
on the future state of the system. Specific instance of optimal
control:allows real-time optimal control over a receding time
horizon. Application to biped walking \cite{herdt2010online},
generalized biped walking \cite{coros2010generalized}, most impressive
results in \cite{tassa2012synthesis}.

* Allow human behaviors found to emerge: walking
\cite{chevallereau2001optimal}, running \cite{Schultz2010}, emotional
walking \cite{felis2012modeling}, generalized locomotion
\cite{coros2010generalized,mordatch2012discovery}

* Simulate disabilities and study their effect on human motion in a
biomechanics context \cite{Lengagne2010}

* We notice convergence of computer graphics, biomechanics and
robotics and simulation models become more and more realistic in order
to generate natural motions easily.
...
results were successfully obtained for simple mechanisms such as 7-DoF
arms; as they do not allow enforcing complicated constraints such as
closed-loop constraints or dynamic balance, achieving optimal motion
planning for anthropomorphic systems with such methods seems to be
impossible.
